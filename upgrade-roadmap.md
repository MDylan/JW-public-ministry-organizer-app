...
- [x] **TODO 10.1: Decide whether to activate `GroupDayObserver`** - DONE
  ...
  - Loose end for anyone who revisits this: `GroupDayObserver::forceDeleted()` calls `GroupDayDeletedProcess::dispatch([...])` with an **array as a single argument** where six are expected - an `ArgumentCountError`. Unreachable today, because `GroupDay` has no `SoftDeletes` and therefore no `forceDelete()`.
  - Expected changes: none in `app/` - this TODO is evidence and documentation.

- [ ] **TODO 10.2: Settle the commented-out purge in `GroupDayDeletedProcess` - the suite is red because of it**
  - Commit `e3633a0b` ("TODO 10.1 fix") commented out the last two live statements in `GroupDayDeletedProcess::handle()`:
    ```php
    // GroupDate::where('id', '=', $day->id)->delete();
    // DayStat::where('day', '=', $day->date)->delete();
    ```
    The enclosing `foreach` over `$groupdates` now has an empty body, so the job still runs its query and then does nothing with it.
  - **`GroupDayJobsTest::test_deleted_process_purges_group_dates_and_stats_for_the_weekday` fails as a direct result**, and has done since that commit. It reproduces in isolation and is unrelated to TODO 11. This is the only red test in the suite.
  - The change is consistent with the TODO 10.1 conclusion - the job is a superseded implementation, and `CalculateDatesEvents` already purges `group_dates` and `day_stats` on the live chain - but it leaves the code and its test disagreeing. Two ways to close it, and the choice is a decision, not a cleanup:
    1. **Keep the deletes commented out** and rewrite the test to pin the new behaviour: the job deletes events and notifies, but no longer touches `group_dates`/`day_stats`. Consistent with `GroupDayUpdatedProcess`, whose body is also fully commented out.
    2. **Restore the two lines.** The job is unreachable in production either way (its only dispatcher is the unregistered observer), so the deletes cost nothing while they sit there, and the job stays self-contained if the observer is ever registered.
  - Whichever is chosen, `.docs/observers.md` needs the same statement, and TODO 05's "Inactive" note should record that these jobs are now partially gutted rather than merely dormant.
  - Expected changes: either `app/Jobs/GroupDayDeletedProcess.php` or `tests/Feature/Jobs/GroupDayJobsTest.php`, plus `.docs/observers.md`.
...
- [ ] **TODO 11.1: Build the `detachParentGroup()` notification payload after the authorization check**
  ...
  - Expected changes: move the `$data` assembly below the guard; no test change should be needed.
