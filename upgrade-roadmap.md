...
- [ ] **TODO 12.1: Replace the two `dd()` calls in the exception handler**
  - Found by TODO 12. `app/Exceptions/Handler.php:52` and `:60` call `dd()` inside `renderable()` callbacks. On an installed site, **any** `QueryException` therefore prints the raw database message to the browser and exits - no error page, no logging, no reporting, and the message may disclose schema or credentials. The `MissingAppKeyException` branch does the same when `.env.example` is absent.
  - This is untestable as written: `dd()` calls `exit`, so a test that reached it would terminate the PHPUnit process. That property alone disqualifies it from a production error path.
  - Needed: let the exception fall through to the normal handler (or `report()` it and render the standard error view). Once it no longer exits, add the coverage that `InstallerExceptionHandlerTest` cannot write today.
  - Expected changes: `app/Exceptions/Handler.php`, extended `tests/Feature/Setup/InstallerExceptionHandlerTest.php`.

- [ ] **TODO 12.2: Make anonymization conditional on succession, not on role**
