...
- [ ] **TODO 10.1: Decide whether to activate `GroupDayObserver`**
  ...
  - Needed:
    ...
    - `tests/Feature/Jobs/GroupDayJobsTest.php` already covers the two job bodies, so only the wiring needs new coverage. Note that those tests all pass a **real** causer; the system-causer (`0`) behaviour lives in `tests/Feature/Jobs/SystemCauserJobsTest.php`.
    - **Beware the sync-queue blind spot when writing those tests.** `phpunit.xml` sets `QUEUE_CONNECTION=sync`, so a dispatched job runs inside the authenticated request and `auth()` still answers - which is precisely how the causer defect described in TODO 10 slipped through. Production runs `QUEUE_CONNECTION=database`. Exercise the job bodies directly, unauthenticated, rather than relying on a dispatch assertion.
