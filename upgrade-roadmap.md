...
- [x] **TODO 10.1: Decide whether to activate `GroupDayObserver`** - DONE
  ...
  - **Decision (the user's): leave it unregistered, and document why.** The observer and its two jobs are a **superseded implementation**, not a missing feature. Registering them would not add a capability - it would run the same cleanup a second time, on a different schedule. `app/Providers/EventServiceProvider.php` is unchanged; the observer and both jobs stay in place with their existing coverage.
