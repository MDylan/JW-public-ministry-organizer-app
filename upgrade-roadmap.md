...
- [ ] **TODO 35.1: The committed `vendor/` tree is incomplete, and no release can be built from this branch until it is fixed**
  ...
  - Expected changes: `.gitignore` and/or `release/build-update.php`, plus one large `vendor/` catch-up commit.
...
- [ ] **TODO 75: Prepare the rollback and deployment checklist**
  - **TODO 35.1 blocks this item outright.** The committed `vendor/` tree is missing 1037 files, so no archive built from this branch - 2.0.0 included, whichever branch section 6 chooses - would contain a working `vendor/`. Close that first; nothing here can be verified on a real host until it is.
