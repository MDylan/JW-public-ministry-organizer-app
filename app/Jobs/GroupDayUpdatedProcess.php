...
class GroupDayUpdatedProcess implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if(count($dates)) {
            CalculateDatesEvents::generate($this->group_id, $dates, $this->user_id);
        }

            // $start = strtotime($event->day." ".$this->start_time);
            // $end_date = $event->day;
            // if(strtotime($this->end_time) == strtotime("00:00")) {
            //     $end_date = Carbon::parse($event->day)->addDay()->format("Y-m-d");
            // }
            // $end = strtotime($end_date." ".$this->end_time);
        ...
        //     $end_date = $day->date;
        //     if(strtotime($this->end_time) == strtotime("00:00")) {
        //         $end_date = Carbon::parse($day->date)->addDay()->format("Y-m-d");
        //     }
        //     $end = strtotime($end_date." ".$this->end_time);
